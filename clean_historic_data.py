...
def clean_historic_data(code_sandre:str):
    """
    Nettoie les données historiques de 1991 à 2020
    :param code_sandre: Filtre les données avec les stations associé à code_sandre.
    """
    ...
    with tqdm(total=total_iterations, desc="Progression dates") as pbar:
        for annee in range(1991,2021):
            for mois in range(1,13):
                ...
                # Clean Hubeau data
                df_hubeau_clean = clean_utils.clean_hubeau_data(annee_mois_jour_filtre,code_sandre)
                chemin_fichier_clean_hubeau = Path(f"output/hubeau/cleaned_data/clean-QmM-{code_sandre}-{annee_mois_filtre}.csv")
...
if __name__ == "__main__":
    ...
    clean_historic_data(sandre_code)
